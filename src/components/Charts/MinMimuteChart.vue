...
<script>
  ...
  import {changeToIndexCode1, changeToIndexCode2, ifCurTimeIsTradeTime} from "../../utils/stockUtils";
    ...
    methods: {
      ...
      getStockHeadInfo(stockCode1) {
        this.$store.dispatch('stock/getStockInfo', stockCode1).then(res => {
          ...
          this.colorStr = getColor(Number(this.stockInfo[3]), Number(this.stockInfo[4]));
        })
      },
      setOptions(stockCode2) {
        this.$store.dispatch('stock/getMinuteStock', stockCode2).then(res => {
          ...
          let interval = minVal > maxVal ? Math.abs((baseNumber - minVal)) : Math.abs((baseNumber - maxVal));
          let bgColor = "#fff";//背景
          ...
          this.chart.setOption({
            ...
            tooltip: { //弹框指示器
              ...
              formatter: function (params) {
                ...
                let html = '<div class="commColor" style="width:90px;">' +
                  '<div>时：<span>' + times[i] + '</span></div>' +
                  '<div>价：<span  ' + color + ' >' + priceArr[i].toFixed(2) + '</span></div>' +
                  '<div>幅：<span  ' + color + ' >' + _this.ratioCalculate(priceArr[i], yestclose) + '%</span></div>';
                return html;
              }
            },
      ...
      handleNum(baseNumber, priceArr) {
        let _aa = Math.abs((Math.max.apply(null, priceArr) - baseNumber) / baseNumber).toFixed(4);
        let _bb = Math.abs((baseNumber - Math.min.apply(null, priceArr)) / baseNumber).toFixed(4);
