  ...
  <div :id="id" :class="className" :style="{height:height,width:width}" :stockCode1="stockCode1">
    <el-row :gutter="5" type="flex" class="el-row-head" align="center">
      ...
      <el-col :span="8">
        ...
        <div class="text-content">今开：<span :style="colorArr[3]">{{stockInfo[5]}}</span></div>
        <div class="text-content">昨收：{{stockInfo[4]}}</div>
        <div class="text-content">换手率：{{stockInfo[38]}}%</div>
      </el-col>
      <el-col :span="8">
        <div class="text-content">最高：<span :style="colorArr[1]">{{stockInfo[33]}}</span></div>
        <div class="text-content">最低：<span :style="colorArr[2]">{{stockInfo[34]}}</span></div>
        <div class="text-content">涨停：<span :style="colorArr[4]">{{stockInfo[47]}}</span></div>
        <div class="text-content">跌停：<span :style="colorArr[5]">{{stockInfo[48]}}</span></div>
      </el-col>
      <el-col :span="8">
        <div class="text-content">最新：<span :style="colorArr[0]">{{stockInfo[3]}}</span></div>
        <div class="text-content">涨跌幅：<span :style="colorArr[0]">{{stockInfo[32]}}%</span></div>
        <div class="text-content">涨跌额：<span :style="colorArr[0]">{{stockInfo[31]}}</span></div>
  ...
  export default {
    ...
    methods: {
      ...
      getStockBasicInfo(stockCode) {
        this.$store.dispatch('stock/getStockBasicInfo', stockCode.substr(2,6)).then(res => {
