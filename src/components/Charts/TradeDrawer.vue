<template>
  <div :id="id" :class="className" :stockCode1="stockCode1" :btnSize="btnSize">
    <el-button type="danger" plain @click="clickBuy" :size="btnSize">买入</el-button>
    <el-button type="primary" plain @click="clickSell" :size="btnSize">卖出</el-button>
    <el-drawer
      :modal="modal"
      :visible.sync="drawer"
      size="49%"
      ...
      <!--头部-->
      <div slot="title">
        <span class="trade-drawer-head-text">总资产：{{totalBalance + totalMarketValue}}</span>
        <span class="trade-drawer-head-text">可用资金：{{avlBalance}}</span>
        <span class="trade-drawer-head-text">参考市值：{{totalMarketValue}}</span>
        <span class="trade-drawer-head-text">浮动盈亏：<span :style="totalFloatingPLColor">{{totalFloatingPL}}</span></span>
        <el-button size="medium" style="margin-left:8%;height: 35px" type="primary" @click="getUserBalanceStatus" round>
          刷新状态
        </el-button>
      </div>
          ...
          <el-tabs v-model="actionName" type="card">
            <el-tab-pane label="挂单" name="entrustOrder">
              ...
              >
                ...
                <el-table-column label="委托时间" prop="costPrice" align="center" width="110">
                  <template slot-scope="{row}">
                    <span>{{ row.entrustTime }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="委托数量" prop="totalNumber" align="center" width="110">
                  <template slot-scope="{row}">
                    <span>{{ row.entrustNumber }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="委托价格" prop="entrustPrice" align="center" width="110">
                  <template slot-scope="{row}">
                    <span>{{ row.entrustPrice }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="委托类型" prop="type" align="center" width="100">
                  <template slot-scope="{row}">
                    <span>{{ row.type }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="委托状态" prop="station" align="center" width="110">
                  <template slot-scope="{row}">
            ...
            <el-tab-pane label="持仓信息" name="holdShares">
              ...
              >
                ...
                <el-table-column label="股票名称" prop="stockName" align="center" width="85">
                  <template slot-scope="{row}">
                    <span>{{ row.stockName }}</span>
                  </template>
                </el-table-column>
                ...
                <el-table-column label="总数量" prop="totalNumber" align="center" width="90">
                  <template slot-scope="{row}">
                ...
                <el-table-column label="浮动盈亏" prop="floatingPL" align="center" width="100">
                  <template slot-scope="{row}">
                ...
                <el-table-column label="现价" prop="nowPrice" align="center" width="80">
                  <template slot-scope="{row}">
                    <span>{{ row.nowPrice }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="市值" prop="marketValue" align="center" width="100">
                  <template slot-scope="{row}">
                    <span>{{ row.marketValue }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="仓位" prop="storeRate" align="center" width="76">
                  <template slot-scope="{row}">
            ...
            <el-tab-pane label="当日委托" name="todayEntrust">
              ...
              >
                ...
                <el-table-column label="成交价格" prop="donePrice" align="center" width="110">
                  <template slot-scope="{row}">
...
<script>
  import {
    ...
    getColorByOne,
    getDownColor,
    getUpColor, handleTimestamp, handleOrderStation, ifCurTimeIsTradeTime
  ...
  export default {
    ...
    methods: {
      ...
      getTodayUserTradeOrderInfo() {
        ...
        this.todayDoneOrder = [];
        this.$store.dispatch('trade/getTodayUserTradeOrderInfo', query).then(response => {
