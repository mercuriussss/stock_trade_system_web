<template>
  <div :id="id" :class="className" :style="{height:height,width:width}" :stockCode1="stockCode1">
    <el-row :gutter="5" type="flex" class="el-row-trade" align="center">
      <el-col :span="12">
        <div class="trade-head-text">委差：
          <div :style="colorArr[10]">{{entrustDValue}}</div>
        </div>
        <div class="trade-head-text">外盘：
          <div :style="colorArr[11]">{{BPan}}</div>
        </div>
      </el-col>
      <el-col :span="12">
        <div class="trade-head-text">委比：
          <div :style="colorArr[10]">{{entrustRatio}}%</div>
        </div>
        <div class="trade-head-text">内盘：
    ...
    <el-row :gutter="5" type="flex" class="el-row-trade-record" align="center">
      ...
      <el-col :span="10">
        <div class="BS-text" v-for="(item) in tradeRecordInfo" :style="item[7]">{{item[2]}}</div>
      </el-col>
      <el-col :span="10">
        <div class="BS-text" v-for="(item) in tradeRecordInfo">{{item[4]}}</div>
      </el-col>
      <el-col :span="3">
        <div class="BS-text" v-for="(item) in tradeRecordInfo" :style="item[8]">{{item[6]}}</div>
      </el-col>
    </el-row>
  </div>
</template>
...
<script>
  import {
    getBaseColor, getBSTradeInfo, getBSTradeInfoColor,
    getColor, getCurPage,
    getDownColor,
    getUpColor,
    handleBSPan, ifCurTimeIsTradeTime,
    ...
    methods: {
      ...
      getStockTradeInfo(stockCode) {
        this.$store.dispatch('stock/getStockInfo', stockCode).then(res => {
          ...
          let entrustBuy = Number(this.stockInfo[10]) + Number(this.stockInfo[12]) + Number(this.stockInfo[14]) + Number(this.stockInfo[16]) + Number(this.stockInfo[18]);
          let entrustSell = Number(this.stockInfo[20]) + Number(this.stockInfo[22]) + Number(this.stockInfo[24]) + Number(this.stockInfo[26]) + Number(this.stockInfo[28]);
          this.entrustDValue = entrustBuy - entrustSell;
          this.entrustRatio = ((entrustBuy - entrustSell) / (entrustBuy + entrustSell) * 100).toFixed(2);
